...
`dashboard/` is a Vite + React + TypeScript app styled with a copy of the frontend's theme. It has no server of its own: `bun run build` writes static files to `dashboard/dist`, the Docker image builds them in a `bun` stage, and the public app serves them from `/`. Python stays the only thing that reads the database. Without a build, nothing is mounted and `/` answers with JSON, so the Python tests never need bun.

```bash
# terminal 1 — the API the app calls (8080 is the features service locally)
uvicorn main_public:app --port 8002

# terminal 2 — Vite on :5173, proxying /v1 to :8002, so one origin and no CORS
cd dashboard && bun install
VITE_DEV_TOKEN=<pipeline token> bun run dev    # the variable is optional: it skips the token prompt, dev only
```
...
**After changing a response model the dashboard reads**, refresh the snapshot and the types in the same commit — CI's `dashboard` job re-exports and fails on a difference:

```bash
python scripts/export_openapi.py > dashboard/openapi/openapi.json
(cd dashboard && bun run generate:api)
```

Response models the dashboard consumes extend `schemas.common.ApiModel`, so fields with defaults are `required` in the schema and come out as `field: T | null` rather than `field?: T | null`.

**Pages.** Each page is a client-side route in `src/App.tsx`, a nav entry in `src/components/shell/nav.ts`, and its own `/v1/dashboard/*` endpoint, so a slow query on one page never slows another's poll:

| Route | Endpoint | What it shows |
|---|---|---|
...
| `/data` Data | `/freshness` (60 s) | Every table a pipeline writes (`PipelineConfig.target_table`): the date it runs through, its last write, and whether that is what the season expects. Nightly tables are judged in season against the last settled game date; live and scheduled writers are shown, not judged |

**Theme.** `dashboard/src/styles/tokens.css` and `dashboard/tailwind.config.ts` are copies of the frontend's, generated by a guard in the manner of the backend mirror guard. Never edit them here:

```bash
python scripts/check_frontend_theme.py            # expects ../frontend; exit 2 = no checkout
python scripts/check_frontend_theme.py --write    # re-sync after the frontend's theme changes
```

As with the mirror guard, no CI runs it (CI has no frontend checkout).
...
| `GET` | `/v1/dashboard/freshness` | Token | What date each pipeline's table runs through, when it was last written, and a verdict (`fresh`, `stale`, `idle`, `empty`, `unjudged`, `error`) against the season calendar and the last settled game date (`services/freshness_service.py`) |
